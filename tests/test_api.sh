...
host1="$(jq -r '.hostname' /tmp/simplelab-vm1.json)"; [[ -n "$host1" && "$host1" != null ]] || fail "first hostname missing"
jq -e '.puppet | has("server_ip")' /tmp/simplelab-vm1.json >/dev/null || fail "puppet.server_ip missing from onboarding response"
pass "first onboarding: $host1"
